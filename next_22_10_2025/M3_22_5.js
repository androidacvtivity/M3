...
//-------------------------------------------------------------------
    ...
    function validate_rule_03064(param, index, parentIndex) {
        if (!param || !param.fieldTemplate) {
            return;
        }

        // Rulăm doar pentru șabloanele statice (R01) ca să nu dublăm erorile
        if (param.fieldTemplate.indexOf('R01') === -1) {
            return;
        }
        ...
        // Verifică în structura returnată de get_field_value dacă există
        // cel puțin o valoare nenulă (merge și pe scalar, și pe array / array de array).
        function valueHasNonZero(v) {
            if (Array.isArray(v)) {
                for (var i = 0; i < v.length; i++) {
                    if (valueHasNonZero(v[i])) {
                        return true;
                    }
                }
                return false;
        ...
        // Caută dacă, pentru un prefix + interval de coloane + sufix (opțional),
        // există cel puțin o valoare nenulă în TOT ce întoarce get_field_value.
        function hasNonZeroOnRow(prefix, colFrom, colTo, suffix) {
            suffix = suffix || '';
            for (var c = colFrom; c <= colTo; c++) {
                var fieldName = prefix + c + suffix;

                // IMPORTANT: nu mai trecem index / parentIndex aici!
                if (typeof field_exists === 'function' && field_exists(fieldName)) {
                    var v = get_field_value(fieldName);
                    if (valueHasNonZero(v)) {
                        return true;
                    }
                }
            }
        ...
        if (table === 'I.') {
            var colFromI = 2;
            var colToI = 10; // COL2..COL10 ca în SQL

            // unitate principală: CAPIa_R00_T_C2..C10 vs CAPIa_R01_T_C2..C10
            var has00_main =
                hasNonZeroOnRow('CAPIa_R00_T_C', colFromI, colToI, '');
            var has01_main =
                hasNonZeroOnRow('CAPIa_R01_T_C', colFromI, colToI, '');

            if (has00_main && !has01_main) {
                pushWarn('CAPIa_R01_T_C1');
            }

            // filiale: CAPIa_R00_T_C2..C10_FILIAL vs CAPIa_R01_T_C2..C10_FILIAL
            var has00_fil =
                hasNonZeroOnRow('CAPIa_R00_T_C', colFromI, colToI, '_FILIAL');
            var has01_fil =
                hasNonZeroOnRow('CAPIa_R01_T_C', colFromI, colToI, '_FILIAL');

            if (has00_fil && !has01_fil) {
                pushWarn('CAPIa_R01_T_C1_FILIAL');
            }
        ...
        if (table === 'II.') {
            var colFromII = 2;
            var colToII = 11; // COL2..COL11 ca în SQL

            // unitate principală: CAPII_R00_T_C2..C11 vs CAPII_R01_T_C2..C11
            var has00_main2 =
                hasNonZeroOnRow('CAPII_R00_T_C', colFromII, colToII, '');
            var has01_main2 =
                hasNonZeroOnRow('CAPII_R01_T_C', colFromII, colToII, '');

            if (has00_main2 && !has01_main2) {
                pushWarn('CAPII_R01_T_C1');
            }

            // filiale: CAPII_R00_T_C2..C11_FILIAL vs CAPII_R01_T_C2..C11_FILIAL
            var has00_fil2 =
                hasNonZeroOnRow('CAPII_R00_T_C', colFromII, colToII, '_FILIAL');
            var has01_fil2 =
                hasNonZeroOnRow('CAPII_R01_T_C', colFromII, colToII, '_FILIAL');

            if (has00_fil2 && !has01_fil2) {
                pushWarn('CAPII_R01_T_C1_FILIAL');
            }
