...
//-------------------------------------------------------------------
    ...
    /**
   * 03-064
   * Cap. I / Cap. II, tab-uri și filiale:
   *
   * Dacă există cel puțin o valoare nenulă pe rândul "Total salariați" (R00)
   * în coloanele 2..10, atunci trebuie să existe cel puțin o valoare nenulă
   * și pe rândul "Total salariați" (R01) în coloanele 2..10, pentru:
   *   - tab-ul principal (CAEM principal)
   *   - fiecare filiala CUATM (filiale)
   */
    function validate_rule_03064(param, index, parentIndex) {
        ...
        // --- Helpers numerice (convertesc null/undefined/"" în 0) ----------------
        function toNumber(v) {
        ...
        /**
         * Verifică dacă pe un rând (Total = index 0) există cel puțin o valoare nenulă
         * în coloanele [colFrom..colTo], pentru câmpuri de forma:
         *   prefix + col + filialSuffix
         * Exemplu: 'CAPIa_R00_T_C' + 2 + ''  => CAPIa_R00_T_C2
         *          'CAPIa_R00_T_C' + 2 + '_FILIAL-2' => CAPIa_R00_T_C2_FILIAL-2
         *
         * Pentru tab principal: filialSuffix = ''
         * Pentru filiale:      filialSuffix = '_FILIAL-2', iar rowIndex este indexul filialei.
         */
        function hasNonZeroOnRow(prefix, colFrom, colTo, rowIndex, filialSuffix) {
            var sum = 0;
            for (var c = colFrom; c <= colTo; c++) {
                var fieldName = prefix + c + filialSuffix;
                var colData = param[fieldName];

                if (!colData) {
                    continue;
                }

                // tab principal: colData este un array de rânduri, luăm rândul 0
                // filiale: colData este un array de filiale, fiecare fiind un array de rânduri
                var row;
                if (filialSuffix === '') {
                    // tab principal
                    ...
                    row = colData[rowIndex] || colData[0]; // de regulă, rândul Total = 0
                } else {
                    // filiale: colData[filialIndex] => array de rânduri pentru filiala respectivă
                    if (!Array.isArray(colData)) {
                        continue;
                    }
                    var filArr = colData[rowIndex];
                    if (!filArr) {
                        continue;
                    }
                    row = filArr; // tot rândul Total este poziția 0 în acel sub-array
                }
                ...
                var v = toNumber(row[0]); // Total salariați = primul element de pe rând
                sum += v;
            }
            return sum !== 0;
        }

        // --- Helper pentru mesaj + deduplicare -----------------------------------
        function pushWarn(tableLabel, filialContext, anchorField, rowIndex, parentIdx) {
            if (!window._rule03064_seen) {
                window._rule03064_seen = {};
            }

            // Deducem cheia unică după tabel + anchorField + context filiala
            var ctxStr = (filialContext || '').toString();
            var key = tableLabel + '|' + anchorField + '|' + ctxStr;

            if (window._rule03064_seen[key]) {
                return;
            }
            window._rule03064_seen[key] = true;

            // Textul specific al regulii (fără cod – codul îl pune generateMessageTitle)
            // Exemplu final:
            //  "Cod eroare: 03-064, Cap. I. (tab. №7), Rândul "Total salariați", Col.1 -
            //   Cap I., Rândul 1 Total – lipsesc datele în CAEM principal (filiala CUATM 2)"
            var msgBody;

            if (filialContext) {
                msgBody = Drupal.t('Total – lipsesc datele în CAEM principal (@ctx)', {
                    '@ctx': filialContext
                });
            } else {
                msgBody = Drupal.t('Total – lipsesc datele în CAEM principal');
            }

            // generateMessageTitle va construi:
            // "Cod eroare: 03-064, Cap. I. (tab. №x), Rândul ..., Col.... - " + msgBody
            var fullMsg = generateMessageTitle('03-064', msgBody, anchorField, rowIndex, parentIdx);
            warnings.push(fullMsg);
        }

        // --- Config comună: coloane 2..10 ----------------------------------------
        var colFrom = 2;
        var colTo = 10; // conform specificației: C2..C10

        // =========================
        //   CAP. I – tab principal
        // =========================
        var has00_I_main = hasNonZeroOnRow('CAPIa_R00_T_C', colFrom, colTo, 0, '');
        var has01_I_main = hasNonZeroOnRow('CAPIa_R01_T_C', colFrom, colTo, 0, '');

        if (has00_I_main && !has01_I_main) {
            // anchorField trebuie să indice rândul R01, coloana 2 din Cap. I
            pushWarn('I.', null, 'CAPIa_R01_T_C2', 0, null);
        }

        // =========================
        //   CAP. I – filiale
        // =========================
        var filListI = param['CAPIa_CUATM_R_INDEX_FILIAL-2'];

        if (Array.isArray(filListI)) {
            for (var f = 0; f < filListI.length; f++) {
                // Pentru filiale folosim același prefix, dar cu sufix '_FILIAL-2'
                var has00_I_fil = hasNonZeroOnRow('CAPIa_R00_T_C', colFrom, colTo, f, '_FILIAL-2');
                var has01_I_fil = hasNonZeroOnRow('CAPIa_R01_T_C', colFrom, colTo, f, '_FILIAL-2');

                if (has00_I_fil && !has01_I_fil) {
                    var ctxI = Drupal.t('filiala CUATM @cuatm', { '@cuatm': filListI[f] });
                    // anchorField pentru filiale: câmpul R01, col.2, varianta FILIAL-2
                    pushWarn('I.', ctxI, 'CAPIa_R01_T_C2_FILIAL-2', 0, f);
                }
            }
        }

        // =========================
        //   CAP. II – tab principal
        // =========================
        var has00_II_main = hasNonZeroOnRow('CAPII_R00_T_C', colFrom, colTo, 0, '');
        var has01_II_main = hasNonZeroOnRow('CAPII_R01_T_C', colFrom, colTo, 0, '');

        if (has00_II_main && !has01_II_main) {
            pushWarn('II.', null, 'CAPII_R01_T_C2', 0, null);
        }

        // =========================
        //   CAP. II – filiale
        // =========================
        var filListII = param['CAPII_CUATM_R_INDEX_FILIAL-2'];

        if (Array.isArray(filListII)) {
            for (var f2 = 0; f2 < filListII.length; f2++) {
                var has00_II_fil = hasNonZeroOnRow('CAPII_R00_T_C', colFrom, colTo, f2, '_FILIAL-2');
                var has01_II_fil = hasNonZeroOnRow('CAPII_R01_T_C', colFrom, colTo, f2, '_FILIAL-2');

                if (has00_II_fil && !has01_II_fil) {
                    var ctxII = Drupal.t('filiala CUATM @cuatm', { '@cuatm': filListII[f2] });
                    pushWarn('II.', ctxII, 'CAPII_R01_T_C2_FILIAL-2', 0, f2);
                }
            }
        }
    }
