...
//------------------------------------------------------------------
    ...
    function validate_rule_03013_1(param, index, parentIndex) {
        ...
        function fieldVal(fieldName) {
            ...
            if (!field_exists(fieldName, index, parentIndex)) {
                return '';
            }
            return get_field_value(fieldName, index, parentIndex);
        }

        // Dacă nu există niciunul din câmpuri, nu avem ce verifica
        if (!field_exists(param.c1_T, index, parentIndex) &&
            !field_exists(param.c1_F, index, parentIndex) &&
            !field_exists(param.c6_T, index, parentIndex) &&
            !field_exists(param.c6_F, index, parentIndex)) {
            return;
        }

        var c1T_raw = fieldVal(param.c1_T);
        var c1F_raw = fieldVal(param.c1_F);
        var c6T_raw = fieldVal(param.c6_T);
        var c6F_raw = fieldVal(param.c6_F);

        // rând considerat completat doar dacă există ceva pe Col.1 sau Col.6 (T sau F)
        var rowFilled = (
        ...
        if (!rowFilled) {
            return; // nu validăm rândul complet gol
        }
        ...
        var denom = c1T - c1F;
        if (!denom) {
            return; // împărțire la 0 sau denom. 0 – nu validăm
        }
        ...
        if (rez < 8 || rez > 10) {
            // ancorăm pe c6_T dacă există, altfel pe alt câmp disponibil
            var anchor = param.c6_T && field_exists(param.c6_T, index, parentIndex) ? param.c6_T
            ...
            var msg = Drupal.t(
                ...
                {
                    '@row': getRowFromFieldName(anchor, index),
                    '@result': formatNumber(rez, 2)
                }
            );
            ...
            webform.warnings.push({
                ...
                weight: 131,
                options: { hide_title: true },
                msg: generateMessageTitle('03-013.1', msg, anchor, index, parentIndex)
            });
        }
    }
