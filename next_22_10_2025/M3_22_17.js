...
//-------------------------------------------------------------------
    ...
    function validate_rule_03064(param, index, parentIndex) {
        var values = (Drupal.settings.mywebform && Drupal.settings.mywebform.values) || {};
        var table = param.table; // "I." sau "II."

        // Rulăm regula o singură dată pe client, chiar dacă e apelată de mai multe ori
        if (window._rule03064_done) {
            return;
        }
        window._rule03064_done = true;
        ...
        // Verifică dacă pe rând (00-T sau 01-T) există cel puțin o coloană ≠ 0
        // prefix: "CAPIa_R00_T_C" / "CAPIa_R01_T_C" / "CAPII_R00_T_C" / "CAPII_R01_T_C"
        // colFrom / colTo: limitele coloanelor (2..10, 2..11 etc.)
        // rowIndex: de obicei 0 (rândul "Total")
        // filialIndex: null = unitatea principală, >=0 = index filiala
        function hasNonZero(prefix, colFrom, colTo, rowIndex, filialIndex) {
            for (var c = colFrom; c <= colTo; c++) {
                var fieldName = prefix + c + (filialIndex != null ? '_FILIAL' : '');
                var val = get_field_value(fieldName, rowIndex, (filialIndex != null ? filialIndex : parentIndex));
                if (toNum(val) !== 0) {
                    return true;
                }
            }
        ...
        // Adăugăm warning 03-064 cu deduplicare
        function push03064(capLabel, anchorField, filialIndex, cuatm) {
            if (!window._rule03064Seen) {
                window._rule03064Seen = {};
            }
            var key = '03064|' + capLabel + '|' + anchorField + '|' +
                (filialIndex == null ? 'MAIN' : ('FILIAL-' + filialIndex));

            if (window._rule03064Seen[key]) {
                return; // deja am adăugat acest mesaj
            }
            window._rule03064Seen[key] = true;

            var ctxFil = '';
            if (cuatm) {
                ctxFil = Drupal.t(' (filiala CUATM @cuatm)', { '@cuatm': cuatm });
            }

            // Dacă nu vrei să calculezi exact tab-ul, poți scoate complet partea cu tab. №
            var tabNo = get_tab_number_from_field(anchorField, (filialIndex != null ? filialIndex : parentIndex));

            var baseMsg = Drupal.t(
                'Cap @cap. (tab. №@tab), Rândul "Total salariați", Col.1 - Cap @cap., Rândul 1 Total – lipsesc datele în CAEM principal@fil',
                {
                    '@cap': capLabel,
                    '@tab': tabNo,
                    '@fil': ctxFil
            ...
            var idx = 0;
            var pIdx = (filialIndex != null ? filialIndex : 0);

            webform.warnings.push({
                fieldName: anchorField,
                index: idx,
                parentIndex: pIdx,
                msg: generateMessageTitle('03-064', baseMsg, anchorField, idx, pIdx)
            });
        }
        ...
        // ==========================
        if (table === 'I.') {
            // CAPIa: col.2..10 (C2..C10)
            var colFromI = 2;
            var colToI = 10;

            // --- Unitatea principală ---
            var has00_main_I = hasNonZero('CAPIa_R00_T_C', colFromI, colToI, 0, null);
            var has01_main_I = hasNonZero('CAPIa_R01_T_C', colFromI, colToI, 0, null);

            if (has00_main_I && !has01_main_I) {
                // câmp-ancoră pe Total principal
                push03064('I.', 'CAPIa_R01_T_C2', null, null);
            }

            // --- Filiale Cap I ---
            var cuatmListI = values.CAPIa_CUATM_R_INDEX_FILIAL || [];
            if (Array.isArray(cuatmListI) && cuatmListI.length > 0) {
                for (var f = 0; f < cuatmListI.length; f++) {
                    var has00_fil_I = hasNonZero('CAPIa_R00_T_C', colFromI, colToI, 0, f);
                    var has01_fil_I = hasNonZero('CAPIa_R01_T_C', colFromI, colToI, 0, f);

                    if (has00_fil_I && !has01_fil_I) {
                        var cuatmI = cuatmListI[f];
                        // câmp-ancoră pe Total filiala (model CAPIa_R01_T_C2_FILIAL)
                        push03064('I.', 'CAPIa_R01_T_C2_FILIAL', f, cuatmI);
                    }
                }
            }
        }
        ...
        // ==========================
        if (table === 'II.') {
            // CAPII: în SQL era până la COL11 — dacă în formă sunt mai puține,
            // get_field_value va întoarce null și toNum() le va trata ca 0.
            var colFromII = 2;
            var colToII = 11; // poți ajusta la 10 dacă vrei strict C2..C10

            // --- Unitatea principală ---
            var has00_main_II = hasNonZero('CAPII_R00_T_C', colFromII, colToII, 0, null);
            var has01_main_II = hasNonZero('CAPII_R01_T_C', colFromII, colToII, 0, null);

            if (has00_main_II && !has01_main_II) {
                push03064('II.', 'CAPII_R01_T_C2', null, null);
            }

            // --- Filiale Cap II ---
            var cuatmListII = values.CAPII_CUATM_R_INDEX_FILIAL || [];
            if (Array.isArray(cuatmListII) && cuatmListII.length > 0) {
                for (var f2 = 0; f2 < cuatmListII.length; f2++) {
                    var has00_fil_II = hasNonZero('CAPII_R00_T_C', colFromII, colToII, 0, f2);
                    var has01_fil_II = hasNonZero('CAPII_R01_T_C', colFromII, colToII, 0, f2);

                    if (has00_fil_II && !has01_fil_II) {
                        var cuatmII = cuatmListII[f2];
                        push03064('II.', 'CAPII_R01_T_C2_FILIAL', f2, cuatmII);
                    }
                }
            }
        }
