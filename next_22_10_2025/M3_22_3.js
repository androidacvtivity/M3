...
//-------------------------------------------------------------------
    ...
    function validate_rule_03064(param, index, parentIndex) {
        if (!param || !param.fieldTemplate || !param.table) return;

        function hasValueSimple(fieldName) {
        ...
        // Mesaj generic, completat cu I. / II. din param.table
        var msg = Drupal.t(
            'Cap @table, Rândul 1 total lipsesc datele în CAEM principal',
            { '@table': param.table }
        );

        // ---------------------------
        // CAP. I : Col.6 sau Col.7
        // ---------------------------
        if (param.table === 'I.') {
            // Verificăm întâi dacă R.1 Total are ORICE date (în alte coloane)
            // ca să nu dăm eroare când rândul e complet gol.
            var rowHasData = false;
            var dataColsI = [2, 3, 4, 5, 6, 7, 8, 9, 10]; // poți extinde până la 11 dacă e cazul

            for (var i = 0; i < dataColsI.length; i++) {
                var col = dataColsI[i];

                var fT = param.fieldTemplate.replace('_TYPE_', 'T').replace('_COL_', col);
                var fF = param.fieldTemplate.replace('_TYPE_', 'F').replace('_COL_', col);

                if (hasValueSimple(fT) || hasValueSimple(fF)) {
                    rowHasData = true;
                    break;
                }
            }

            // dacă rândul 1 este complet gol -> nu se aplică regula
            if (!rowHasData) return;

            // Acum verificăm strict CAEM principal:
            // trebuie să existe Col.6 sau Col.7 (T sau F)
            var hasCaemMain = false;
            var caemCols = [6, 7];

            for (var k = 0; k < caemCols.length; k++) {
                var c = caemCols[k];

                var fT_caem = param.fieldTemplate.replace('_TYPE_', 'T').replace('_COL_', c);
                var fF_caem = param.fieldTemplate.replace('_TYPE_', 'F').replace('_COL_', c);

                if (hasValueSimple(fT_caem) || hasValueSimple(fF_caem)) {
                    hasCaemMain = true;
                    break;
                }
            }
            ...
            if (!hasCaemMain) {
                // ancorăm mesajul pe o coloană vizibilă, de ex. Col.8 T
                var anchorI = param.fieldTemplate.replace('_TYPE_', 'T').replace('_COL_', 8);
                ...
                webform.warnings.push({
                    ...
                    weight: 64,
                    options: { hide_title: true },
                    msg: generateMessageTitle('03-064', msg, anchorI, 0, undefined)
                });
            }
        ...
        // ---------------------------
        if (param.table === 'II.') {
            // Verificăm dacă rândul 1 Total are date în alte coloane (2–8)
            var rowHasDataII = false;
            var dataColsII = [2, 3, 4, 5, 6, 7, 8];

            for (var j = 0; j < dataColsII.length; j++) {
                var colII = dataColsII[j];

                var fT2 = param.fieldTemplate.replace('_TYPE_', 'T').replace('_COL_', colII);
                var fF2 = param.fieldTemplate.replace('_TYPE_', 'F').replace('_COL_', colII);

                if (hasValueSimple(fT2) || hasValueSimple(fF2)) {
                    rowHasDataII = true;
                    break;
                }
            }

            // rândul 1 complet gol -> nu aplicăm regula
            if (!rowHasDataII) return;

            // trebuie să existe CAEM principal pe Col.1 (T sau F)
            var f1T = param.fieldTemplate.replace('_TYPE_', 'T').replace('_COL_', 1);
            var f1F = param.fieldTemplate.replace('_TYPE_', 'F').replace('_COL_', 1);

            var hasCaemMainII = hasValueSimple(f1T) || hasValueSimple(f1F);

            if (!hasCaemMainII) {
                // ancorăm mesajul pe Col.7 T (ca în mesajele tale)
                var anchorII = param.fieldTemplate.replace('_TYPE_', 'T').replace('_COL_', 7);
                ...
                webform.warnings.push({
                    ...
                    weight: 64,
                    options: { hide_title: true },
                    msg: generateMessageTitle('03-064', msg, anchorII, 0, undefined)
                });
            }
