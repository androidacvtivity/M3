...
//-------------------------------------------------------------------
    ...
    // 03-064 / 03-065  – CAEM principal: dacă există date pe 00-T,
    // trebuie să existe cel puțin o coloană nenulă pe 01-T
    function validate_rule_03064(param, index, parentIndex) {
        if (!param || !param.fieldTemplate) {
            return;
        }

        // Rulea este configurată să fie chemată și pe dynamic_section,
        // dar noi vrem să o executăm DOAR pentru șabloanele statice (R01*)
        // ca să nu dublăm erorile.
        ...
        // Verifică dacă pe un "rând" (prefix + [colFrom..colTo] + sufix)
        // există cel puțin o coloană cu valoare nenulă
        function hasNonZeroOnRow(prefix, colFrom, colTo, suffix) {
            suffix = suffix || '';
            for (var c = colFrom; c <= colTo; c++) {
                var fieldName = prefix + c + suffix;
                if (typeof field_exists === 'function' && field_exists(fieldName, 0, null)) {
                    var v = get_field_value(fieldName, 0, null);
                    if (toNum(v) !== 0) {
                        return true;
                    }
                }
        ...
        if (table === 'I.') {
            var colFromI = 2;
            var colToI = 10; // conform SQL: COL2..COL10

            // unitate principală
            var has00_main =
                hasNonZeroOnRow('CAPIa_R00_T_C', colFromI, colToI, ''); //Uncaught TypeError: can't access property "null", value is undefined
            var has01_main =
                hasNonZeroOnRow('CAPIa_R01_T_C', colFromI, colToI, '');

            if (has00_main && !has01_main) {
                // ancorăm pe codul CAEM principal
        ...
        if (table === 'II.') {
            var colFromII = 2;
            var colToII = 11; // conform SQL: COL2..COL11
