...
//But without the comma after the zero here. Here integers not floats
        ...
        function parseFloatWithCharacter(value) {
            return parseFloat(value.replace(/[^\d.]/g, '')); // Remove non-numeric characters using regex
        }

        // Perform comparison after parsing float values and fixing precision
        var parsedValue1 = parseFloatWithCharacter(values.CAPV_R3_1_1_C1).toFixed(1);
        var parsedValue2 = parseFloatWithCharacter(values.CAPV_R3_1_C1).toFixed(1);

        // Check if both parsed values are valid numbers before comparison
        if (!isNaN(parsedValue1) && !isNaN(parsedValue2) && parsedValue1 > parsedValue2) {
            webform.errors.push({
                'fieldName': 'CAPV_R3_1_1_C1',
                'index': 0,
                'weight': 94,
